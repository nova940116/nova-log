...
import InformationAside from "@/components/Information-aside"
...
```
먼저 클라이언트단에서 api를 호출하는 로직을 먼저 구현해보겠습니다. 위의 예시처럼 제목, 내용, 배경색상과 같은 정보들은 querystring으로 넘겨줍니다. 그 후 api에서 넘겨주는 이미지 파일 정보를
blob형태로 받아 image url을 설정하여 화면에 노출합니다.
...
클라이언트단에서 넘겨준 querystring 값을 받아서 이미지의 제목, 내용등으로 구성합니다. HTML + CSS 조합으로 스타일링을 할 수 있습니다. 예시는 간단하게 동작원리만을 설명했지만,
