...
하여 image를 응답 받고, `setImageUrl` 훅을 호출해 변경된 썸네일 이미지를 보여줍니다. 제목과 이름의 경우엔 `change` 이벤트가 일어나도록 해두었기에 debounce를 걸어
잦은 api 호출을 방지했습니다.
...
```
`@vercel/og`는 Next.js 13의 새 기능인 App Router를 사용한다면 따로 설치하지 않아도 사용이 가능합니다.
